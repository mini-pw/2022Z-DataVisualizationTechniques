...
KSBox <- tabPanel(
  ...
  fluidPage(fluidRow(column(4,
                            selectInput('si21',
                                        ...
                                        choices = sort(unique(dfD2KS$FriendNick)),
                                        selected =  sort(unique(dfD2KS$FriendNick))[1])),
                     column(4, selectInput('si22',
                                           shiny::HTML("<p>Select <span style='color: 	#fa3c4c'>friend 2</span></p>"),
                                           choices = sort(unique(dfD2KS$FriendNick)),
                                           selected =  sort(unique(dfD2KS$FriendNick))[2])),
                     column(4, selectInput('si23',
                                           shiny::HTML("<p>Select <span style='color: #EE82EE'>friend 3</span></p>"),
                                           choices = sort(unique(dfD2KS$FriendNick)),
                                           selected =  sort(unique(dfD2KS$FriendNick))[3]))
  ),
  plotlyOutput("acitivityPlot2", height = "418px")
  )
)
...
JPBox <- tabPanel(
  ...
  plotlyOutput("usWithSomeone2", height = "505px")
)
...
wordCountPanel <- tabPanel(
  ...
  tags$div(class = "col",
           actionButton("addButton2", "Add"))
)


dCounterCheckBox2 <- radioGroupButtons(
  inputId = "DRadio2", label = NULL, 
